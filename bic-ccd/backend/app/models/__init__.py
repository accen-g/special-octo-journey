...
Table names align with the 23-table CCB Oracle schema (BIC_CCD_ prefix).
...
class MonthlyControlStatus(AuditMixin, Base):
    ...
    period_year: Mapped[int] = mapped_column("YEAR", Integer, nullable=False)
    period_month: Mapped[int] = mapped_column("MONTH", Integer, nullable=False)
    # status stored as string in our app; status_fk is the BIC FK to BIC_CCD_KRI_STATUS
...
class UserRoleMapping(AuditMixin, Base):
    ...
    Complements BIC_CCD_KRI_USER_ROLE (KRI-level boolean flags).
    """
...
class KriConfiguration(AuditMixin, Base):
    ...
    Not in BIC 23 — BIC stores KRI-level config in BIC_CCD_KRI_CONFIG.
