...
Table names align with the 23-table BIC Oracle schema (BIC_ prefix).
...
class KriCategoryMaster(AuditMixin, Base):
    __tablename__ = "BIC_KRI_CATEGORY"
...
class KriMaster(AuditMixin, Base):
    ...
    region_id: Mapped[int] = mapped_column("REGION_ID", Integer, ForeignKey("BIC_REGION.REGION_ID"), nullable=False)
    category_id: Mapped[int] = mapped_column("CATEGORY_ID", Integer, ForeignKey("BIC_KRI_CATEGORY.CATEGORY_ID"), nullable=False)
...
class MetricValues(AuditMixin, Base):
    __tablename__ = "BIC_KRI_METRIC"

    metric_id: Mapped[int] = mapped_column("METRIC_ID", Integer, Identity(start=1), primary_key=True)
    kri_id: Mapped[int] = mapped_column("KRI_ID", Integer, ForeignKey("BIC_KRI_CONFIG.KRI_ID"), nullable=False)
    dimension_id: Mapped[int] = mapped_column("CONTROL_ID", Integer, ForeignKey("BIC_KRI_CONTROL.CONTROL_ID"), nullable=False)
    ...
    source_id: Mapped[Optional[int]] = mapped_column("SOURCE_ID", Integer, ForeignKey("BIC_KRI_DATA_SOURCE_MAPPING.ID"))
...
class KriComment(AuditMixin, Base):
    ...
    # Extra
    dimension_id: Mapped[Optional[int]] = mapped_column("CONTROL_ID", Integer, ForeignKey("BIC_KRI_CONTROL.CONTROL_ID"))
    status_id: Mapped[Optional[int]] = mapped_column("STATUS_ID", Integer, ForeignKey("BIC_KRI_CONTROL_STATUS_TRACKER.ID"))
    comment_type: Mapped[str] = mapped_column("COMMENT_TYPE", String(20), default="GENERAL")
    parent_comment_id: Mapped[Optional[int]] = mapped_column("PARENT_COMMENT_ID", Integer, ForeignKey("BIC_KRI_COMMENT.ID"))
...
class MonthlyControlStatus(AuditMixin, Base):
    ...
    __tablename__ = "BIC_KRI_CONTROL_STATUS_TRACKER"
    ...
    period_year: Mapped[int] = mapped_column("YEAR", Integer, nullable=False)
    period_month: Mapped[int] = mapped_column("MONTH", Integer, nullable=False)
    # status stored as string in our app; status_fk is the BIC FK to BIC_KRI_STATUS
    status: Mapped[str] = mapped_column("STATUS", String(30), default="NOT_STARTED")
    status_fk: Mapped[Optional[int]] = mapped_column("STATUS_ID", Integer, ForeignKey("BIC_KRI_STATUS.STATUS_ID"))
    ...
    __table_args__ = (
        UniqueConstraint("KRI_ID", "CONTROL_ID", "YEAR", "MONTH", name="uq_bic_tracker"),
        Index("idx_bic_tracker_period", "YEAR", "MONTH"),
    )
...
class EvidenceVersionAudit(Base):
    ...
    __tablename__ = "BIC_KRI_CONTROL_EVIDENCE_AUDIT"

    version_id: Mapped[int] = mapped_column("ID", Integer, Identity(start=1), primary_key=True)
    evidence_id: Mapped[int] = mapped_column("TRACKER_ID", Integer, ForeignKey("BIC_KRI_EVIDENCE.ID"), nullable=False)
...
class EvidenceMetadata(AuditMixin, Base):
    __tablename__ = "BIC_KRI_EVIDENCE"
    ...
    region_id: Mapped[Optional[int]] = mapped_column("REGION_ID", Integer, ForeignKey("BIC_REGION.REGION_ID"))
    ...
    tracker_id: Mapped[Optional[int]] = mapped_column(
        "TRACKER_ID", Integer, ForeignKey("BIC_KRI_CONTROL_STATUS_TRACKER.ID"), nullable=True
    )
...
class KriUserRole(Base):
    ...
    __tablename__ = "BIC_KRI_USER_ROLE"
    ...
    __table_args__ = (
        UniqueConstraint("KRI_ID", "USER_ID", name="uq_kri_user_role"),
    )


# ─── BIC_ROLE_REGION_MAPPING ────────────────────────────────
class RoleRegionMapping(Base):
    """Maps AD group roles to regions."""
    __tablename__ = "BIC_ROLE_REGION_MAPPING"
    ...
    __table_args__ = (
        UniqueConstraint("ROLE", "REGION", name="uq_role_region"),
    )


# ─── BIC_KRI_DATA_SOURCE_MAPPING ────────────────────────────
class DataSourceMapping(AuditMixin, Base):
    __tablename__ = "BIC_KRI_DATA_SOURCE_MAPPING"
...
# ─── BIC_KRI_DATA_SOURCE_STATUS_TRACKER ─────────────────────
class DataSourceStatusTracker(Base):
    """Tracks data receipt status per mapping per month."""
    __tablename__ = "BIC_KRI_DATA_SOURCE_STATUS_TRACKER"

    id: Mapped[int] = mapped_column("ID", Integer, Identity(start=1), primary_key=True)
    mapping_id: Mapped[int] = mapped_column("MAPPING_ID", Integer, ForeignKey("BIC_KRI_DATA_SOURCE_MAPPING.ID"), nullable=False)
    period_month: Mapped[int] = mapped_column("MONTH", Integer, nullable=False)
    period_year: Mapped[int] = mapped_column("YEAR", Integer, nullable=False)
    status_id: Mapped[Optional[int]] = mapped_column("STATUS_ID", Integer, ForeignKey("BIC_KRI_STATUS.STATUS_ID"))
    ...
    __table_args__ = (
        UniqueConstraint("MAPPING_ID", "MONTH", "YEAR", name="uq_ds_tracker"),
    )


# ─── BIC_KRI_ASSIGNMENT_TRACKER ─────────────────────────────
class KriAssignment(AuditMixin, Base):
    __tablename__ = "BIC_KRI_ASSIGNMENT_TRACKER"
...
class AssignmentAudit(Base):
    ...
    assignment_id: Mapped[Optional[int]] = mapped_column("ASSIGNMENT_ID", Integer, ForeignKey("BIC_KRI_ASSIGNMENT_TRACKER.ID"))
...
class ScorecardCase(AuditMixin, Base):
    ...
    file_id: Mapped[Optional[int]] = mapped_column("FILE_ID", Integer, ForeignKey("BIC_CASE_FILE.FILE_ID"))
    region_id: Mapped[int] = mapped_column("REGION_ID", Integer, ForeignKey("BIC_REGION.REGION_ID"), nullable=False)
    ...
    # Extra
    kri_id: Mapped[Optional[int]] = mapped_column("KRI_ID", Integer, ForeignKey("BIC_KRI_CONFIG.KRI_ID"))
...
# ─── BIC_SCORECARD_APPROVER ─────────────────────────────────
class ScorecardApprover(Base):
    __tablename__ = "BIC_SCORECARD_APPROVER"

    approver_id: Mapped[int] = mapped_column("ID", Integer, Identity(start=1), primary_key=True)
    case_id: Mapped[int] = mapped_column("CASE_ID", Integer, ForeignKey("BIC_SCORECARD.TRANSACTION_ID"), nullable=False)
    user_id: Mapped[int] = mapped_column("USER_ID", Integer, ForeignKey("APP_USER.USER_ID"), nullable=False)
...
# ─── BIC_SCORECARD_ACTIVITY_LOG ─────────────────────────────
class ScorecardActivityLog(Base):
    __tablename__ = "BIC_SCORECARD_ACTIVITY_LOG"
...
class CaseFile(Base):
    ...
    __tablename__ = "BIC_CASE_FILE"

    file_id: Mapped[int] = mapped_column("FILE_ID", Integer, Identity(start=1), primary_key=True)
    case_id: Mapped[Optional[int]] = mapped_column("CASE_ID", Integer, ForeignKey("BIC_CASE.TRANSACTION_ID"))
...
class EmailAudit(Base):
    ...
    related_kri_id: Mapped[Optional[int]] = mapped_column("RELATED_KRI_ID", Integer, ForeignKey("BIC_KRI_CONFIG.KRI_ID"))
    related_status_id: Mapped[Optional[int]] = mapped_column("RELATED_STATUS_ID", Integer, ForeignKey("BIC_KRI_CONTROL_STATUS_TRACKER.ID"))


# ─── BIC_SHED_LOCK ──────────────────────────────────────────
class SchedulerLock(Base):
    """ShedLock-compatible distributed scheduler lock."""
    __tablename__ = "BIC_SHED_LOCK"
...
class UserRoleMapping(AuditMixin, Base):
    ...
    Complements BIC_KRI_USER_ROLE (KRI-level boolean flags).
    """
...
class KriConfiguration(AuditMixin, Base):
    ...
    Not in BIC 23 — BIC stores KRI-level config in BIC_KRI_CONFIG.
    ...
    __table_args__ = (
        UniqueConstraint("KRI_ID", "CONTROL_ID", name="uq_kri_dim_cfg"),
    )
...
class MakerCheckerSubmission(AuditMixin, Base):
    ...
    status_id: Mapped[int] = mapped_column("STATUS_ID", Integer, ForeignKey("BIC_KRI_CONTROL_STATUS_TRACKER.ID"), nullable=False)
    evidence_id: Mapped[Optional[int]] = mapped_column("EVIDENCE_ID", Integer, ForeignKey("BIC_KRI_EVIDENCE.ID"))
...
class VarianceSubmission(AuditMixin, Base):
    ...
    metric_id: Mapped[int] = mapped_column("METRIC_ID", Integer, ForeignKey("BIC_KRI_METRIC.METRIC_ID"), nullable=False)
    status_id: Mapped[Optional[int]] = mapped_column("STATUS_ID", Integer, ForeignKey("BIC_KRI_CONTROL_STATUS_TRACKER.ID"))
...
class ApprovalAssignmentRule(AuditMixin, Base):
    ...
    region_id: Mapped[Optional[int]] = mapped_column("REGION_ID", Integer, ForeignKey("BIC_REGION.REGION_ID"))
    kri_id: Mapped[Optional[int]] = mapped_column("KRI_ID", Integer, ForeignKey("BIC_KRI_CONFIG.KRI_ID"))
    category_id: Mapped[Optional[int]] = mapped_column("CATEGORY_ID", Integer, ForeignKey("BIC_KRI_CATEGORY.CATEGORY_ID"))
